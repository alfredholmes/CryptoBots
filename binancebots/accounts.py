...
class USDTFuturesAccount:
	def __init__(self, api, secret, order_book_manager=None, rest_endpoint='https://fapi.binance.com/fapi/v1/', websocket_endpoint='wss://fstream.binance.com/stream'):
		self.httpx_client = httpx.AsyncClient()
		self.api_key = api
		self.secret_key = secret
		if order_book_manager is not None:
			self.orderbook_manager = order_book_manager
	...
	async def get_exchange_data(self):
		...
		for symbol_data in self.exchange_data['symbols']:
			...
			if symbol_data['status'] != 'TRADING':
				continue
			self.market_filters[symbol] = {'precision': int(symbol_data['baseAssetPrecision']), 'precision_quote': int(symbol_data['quotePrecision'])}
			for f in symbol_data['filters']:
				if f['filterType'] == 'LOT_SIZE':
					...
					self.market_filters[symbol]['min_order_quote'] = float(f['notional'])
	...
	async def limit_sell(self, market, quantity):
		pass
